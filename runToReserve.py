...
class Court_Reservation:
    ...
    def confirm(self):
        ...
        check = check and query_yes_no(
            "場地(Place): %s" % place_dict[int(self.place_seq)]
        )
    ...
    def get_form_link(self):
        ...
        if button:
            ...
            log(
                "Not Open!(%d) Try again after %d ms..."
                % (response.status_code, self.retry_interval * 1000)
            )
            time.sleep(self.retry_interval)
            return False, url
...
if __name__ == "__main__":
    ...
    # schedule.every().day.at("09:27").do(court_reservation.login)
    # # court_reservation.login()
    # schedule.every().day.at("09:28").do(court_reservation.reserve)
